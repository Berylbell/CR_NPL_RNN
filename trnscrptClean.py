...
def seperate_Chara(text):
    ...
    for line in lines:
        ...
        txtsplit = line.split(':')
        if(splits == 1):
            speaker.append(txtsplit[0].replace(' ', ''))
            dia.append(txtsplit[1])            
            speak_text.setdefault(txtsplit[0].replace(' ', ''), [])
            speak_text[txtsplit[0].replace(' ', '')].append(txtsplit[1])
            ...
            speaker.append(txtsplit[0].replace(' ', ''))
            dia.append(txtsplit[1])
            #speak_text.setdefault(speaker[-1], [])
            speak_text.setdefault(txtsplit[0].replace(' ', ''), [])
            speak_text[txtsplit[0].replace(' ', '')].append(txtsplit[1])
            #speak_text[txtsplit[0]] = txtsplit[1]
